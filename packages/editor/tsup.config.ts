...
/**
 * @baneung-pack/editor 빌드 설정. (@baneung-pack/grid와 동일 패턴)
 *
 * - 단일 진입점 (index.ts)
 * - ESM + CJS dual + .d.ts
 ...
 * - React/state/ref/contentEditable를 사용하므로 RSC 환경에서 client 필수 → 'use client' 주입
 */
const USE_CLIENT_BANNER = "'use client';\n";

/** React client 런타임 import가 있는 파일에만 'use client' 주입. */
const CLIENT_IMPORT_REGEX = /from\s+["'](react(\/|$|-dom)|@radix-ui\/)/;
